...
class DPPN_gg_endecoder(nn.Module):
  ...
  def __init__(self, att_dim, image_dim, att_hC, T, degree_a, degree_v, att_centroids):
    ...
    #self.encoder     = nn.Linear(image_dim, 256)
    #self.decoder     = nn.Linear(256, image_dim)
    self.encoder     = nn.Linear(image_dim, 512)
  ...
  def vis_propagate(self, protos, attributes):
    enc_protos   = self.encoder(protos)
    _, attention = self.get_attention(attributes)
    prop_protos  = torch.mm(attention, enc_protos)
    dec_protos   = self.decoder(prop_protos)
    return dec_protos
