...
class DPPN_gg_trans(nn.Module):
  """docstring for PPN"""
  def __init__(self, att_dim, img_dim, att_hC, T, degree_a, degree_v, att_centroids):
    super(DPPN_gg_trans, self).__init__()
    ...
    #self.proto_transform   = nn.ModuleList([nn.Linear(img_dim, img_dim), nn.ReLU()])
    self.proto_transform   = nn.ModuleList([nn.Linear(img_dim, img_dim)])
  ...
  def propagate(self, task_attention, protos):
    for m in self.proto_transform:
      protos = m(protos)
    protos = torch.mm(task_attention, protos)
    return protos
